            ...
            try {
                when (c.kind) {
                    ...
                    "enablecs" -> {
                        ...
                        var ymlDoc = tree.listFiles().firstOrNull { it.isFile && it.name?.equals("user.yml", true) == true }
                        if (ymlDoc == null) {
                            val uniStr = prefs.uni()
                            if (uniStr.isNotBlank()) {
                                val uniRoot = DocumentFile.fromTreeUri(this, Uri.parse(uniStr))
                                ymlDoc = uniRoot?.listFiles()?.firstOrNull { it.isFile && it.name?.equals("user.yml", true) == true }
                            }
                        }
                        var done = false
                        if (ymlDoc != null) {
                        ...
                        results.add(c.id to (if (done) null else "User.yml niet gevonden"))
                    }
